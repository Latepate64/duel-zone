...
public abstract class Player : IPlayer, IDisposable
{
    ...
    public List<ICard> DeckCards => Deck.Cards;

    public ICard[] CardsInManaZone => ManaZone.Cards.ToArray();

    public ICard RevealTopCardOfOwnDeck(IGame game) =>
        RevealTopCardsOfDeck(1, game).SingleOrDefault();

    public void PutTopCardOfOwnDeckIntoOwnHand(IGame game, IAbility ability) =>
        game.Move(ability, ZoneType.Deck, ZoneType.Hand, Deck.TopCard);
        
    public void PutTopCardOfOwnDeckIntoOwnGraveyard(
        IGame game, IAbility ability) =>
            game.Move(ability, ZoneType.Deck, ZoneType.Graveyard, Deck.TopCard);
}