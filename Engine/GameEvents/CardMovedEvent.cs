    ...
    public class CardMovedEvent : GameEvent, ICardMovedEvent
    {
        ...
        public override void Happen(IGame game)
        {
            if (Player != null)
            {
                ...
                var removed = (Source == ZoneType.BattleZone ? game.BattleZone : Source == ZoneType.SpellStack ? game.SpellStack : Player.GetZone(Source)).Remove(card);
                foreach (var removedCard in removed)
                {
                    if (Destination != ZoneType.Anywhere)
                    {
                        ...
                        (Destination == ZoneType.BattleZone ? game.BattleZone : Player.GetZone(Destination)).Add(newObject);
