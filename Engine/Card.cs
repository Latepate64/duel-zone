...
abstract public class Card : ICard, ICopyable<ICard>, ITimestampable
{
    ...
    public bool IsDragon => Races.Intersect([Race.ArmoredDragon,
        Race.EarthDragon, Race.VolcanoDragon, Race.ZombieDragon]).Any();
    public bool IsEvolutionCreature => Supertypes.Any(
        x => x == Supertype.Evolution);
    public bool IsMultiColored => Civilizations.Count > 1;
    public bool IsNonEvolutionCreature =>
        CardType == CardType.Creature && !IsEvolutionCreature;
}