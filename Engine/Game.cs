...
public class Game : IDisposable, IGame
{
    ...
    public bool CanAttackAtLeastSomething(ICard creature) =>
        CanAttackAtLeastOneCreature(creature) || CanAttackPlayers(creature);

    public IEnumerable<ICard> GetChoosableCreaturesControlledByAnyone(
        IGame game, Guid id) =>
            BattleZone.GetChoosableCreaturesControlledByAnyone(game, id);

    public IEnumerable<ICard> GetBattleZoneCreatures(IPlayer player) =>
        BattleZone.GetCreatures(player);
    public IEnumerable<ICard> GetBattleZoneCreaturesWithSilentSkill(
        IPlayer player) => BattleZone.GetCreaturesWithSilentSkill(player);
    public void RemoveSummoningSicknesses(IPlayer player) =>
        BattleZone.RemoveSummoningSicknesses(player);
    public bool
        CanPlayerUntapTheCardsInTheirManaZoneAtTheStartOfEachOfTheirTurns(
            IPlayer player) => ContinuousEffects.
                CanPlayerUntapTheCardsInTheirManaZoneAtTheStartOfEachOfTheirTurns(
                    player);
    public bool DoCreaturesInTheBattleZoneUntapAtTheStartOfEachPlayersTurn()
        => ContinuousEffects.DoCreaturesInTheBattleZoneUntapAtTheStartOfEachPlayersTurn();
    public int GetAmountOfBattleZoneCreatures(IPlayer player)
        => GetBattleZoneCreatures(player).Count();
}