...
namespace Cards.OneShotEffects
{
    class YouMayChooseDarknessOrFireCreatureInTheBattleZoneAndTapItEffect : TapChoiceEffect
    {
        ...
        {
            return game.BattleZone.GetChoosableCreaturesControlledByAnyone(game, source.GetOpponent(game).Id).Where(x => x.HasCivilization(Engine.Civilization.Darkness, Engine.Civilization.Fire));
        }
    }
}