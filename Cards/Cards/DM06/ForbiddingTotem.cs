...
namespace Cards.Cards.DM06
{
    ...
    class ForbiddingTotemAbility : ContinuousEffect, ICannotAttackCreaturesEffect, ICannotAttackPlayersEffect
    {
        ...
        public bool CannotAttackCreature(ICard attacker, ICard target, IGame game)
        {
            ...
            if (attacker.Id == game.GetOpponent(GetController(game)).Id)
            {
        ...
        {
            return attacker.Id == game.GetOpponent(GetController(game)).Id && AttackableMysteryTotemExists(attacker, game);
        }
        ...
        {
            return game.BattleZone.GetCreatures(GetController(game).Id, Race.MysteryTotem).Any(x => attacker.CanAttackCreature(x, game));
        }
    }
}
