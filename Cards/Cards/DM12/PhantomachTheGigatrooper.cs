...
namespace Cards.Cards.DM12
{
    ...
    class PhantomachPowerEffect : ContinuousEffect, IPowerModifyingEffect
    {
        ...
        {
            game.BattleZone.GetCreatures(Controller.Id).Where(x => !IsSourceOfAbility(x, game) && (x.HasRace(Race.Chimera) || x.HasRace(Race.Armorloid))).ToList().ForEach(x => x.Power += 2000);
        }
